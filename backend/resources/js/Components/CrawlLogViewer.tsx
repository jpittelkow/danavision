...
import type { CrawlLogEntry, CrawlLogLevel, CrawlJobOutputData } from '@/types';

/**
 * Props for the CrawlLogViewer component.
 */
interface CrawlLogViewerProps {
  /** Output data from the crawl job */
  outputData?: CrawlJobOutputData | Record<string, unknown>;
 ...
 */
function getLogLevelStyle(level: CrawlLogLevel): {
 ...
 */
function convertLegacyLogs(logs: string[]): CrawlLogEntry[] {
  return logs.map((message, index) => {
    // Try to infer log level from message content
    let level: CrawlLogLevel = 'info';
...
/**
 * CrawlLogViewer Component
 *
 * Displays crawl job logs in a console-style format with color coding,
 * timestamps, and expandable/collapsible sections.
 */
export function CrawlLogViewer({
  outputData,
  ...
  const crawlData = outputData as CrawlJobOutputData | undefined;
  const structuredLogs = crawlData?.progress_logs;
  const legacyLogs = crawlData?.logs;
  const crawlStats = crawlData?.crawl_stats;

  // Get logs array (prefer structured, fall back to legacy)
  const logs: CrawlLogEntry[] = structuredLogs?.length
    ? structuredLogs
    : legacyLogs?.length
  ...
  const logCounts = logs.reduce(
    ...
    {} as Record<CrawlLogLevel, number>
  );
    ...
    <div className={cn('mt-3 border rounded-lg bg-muted/30', className)}>
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Crawl Logs</span>
        ...
        <div className="border-t">
          ...
          {crawlStats && (
            <div className="px-3 py-2 bg-muted/50 border-b flex flex-wrap gap-4 text-xs">
              <span>
                <strong>URLs:</strong> {crawlStats.urls_successful}/{crawlStats.urls_attempted} successful
              </span>
