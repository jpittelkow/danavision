...
function ChannelCredentialCard({
  ...
  onToggleAvailable?: (channelId: string, available: boolean) => void;
}) {
  ...
  const toggleAction = adminChannel && adminChannel.admin_toggle ? (
    <div
    ...
    >
      ...
      <Switch
        checked={adminChannel.available}
        onCheckedChange={(checked) => onToggleAvailable?.(adminChannel.id, checked)}
        disabled={!isConfigured || savingToggle}
      />
      <Label className="text-sm text-muted-foreground whitespace-nowrap">Available</Label>
    </div>
  ) : adminChannel && !adminChannel.admin_toggle ? (
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Enable channels and configure credentials. Each card saves independently.
          </p>
        </div>

        {channels.map((ch) => {
          const config = CHANNEL_CONFIGS.find((c) => c.channelId === ch.id);
          if (config) {
            return (
              <ChannelCredentialCard
                key={ch.id}
                ...
                onSaved={fetchConfig}
                savingToggle={savingChannels.has(ch.id)}
                onToggleAvailable={handleToggleAvailable}
              />
            );
          }
          // Channels without credential config (e.g., database, email)
          return (
            <Card key={ch.id}>
              <CardHeader>
