...
**AI-Readable Upgrade Guide:** The changelog page includes an **AI Export** button that generates a structured markdown upgrade guide between any two versions. The export includes YAML frontmatter (version range, format metadata), quick summary with change counts, version-by-version details, consolidated changes by category, database migration detection, and step-by-step instructions for AI agents to apply changes to forked codebases. Admins can configure export format (detailed/summary), detail level (full/changes-only), and instruction style (step-by-step/checklist/minimal) via settings schema (`changelog` group). API: `GET /api/changelog/versions` (available versions), `GET /api/changelog/export?from=X&to=Y` (download markdown). See [journal entry](journal/2026-03-10-ai-changelog-export-settings.md).
