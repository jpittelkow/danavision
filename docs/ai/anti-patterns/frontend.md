...
### Don't: Use a Custom `role="button"` div Instead of `Button`

```tsx
// ❌ WRONG: No keyboard support, no focus ring, no accessible role
<div role="button" onClick={handleClick} className="cursor-pointer p-2">
  <Check className="h-4 w-4" />
</div>

// ✅ CORRECT: shadcn Button handles Enter/Space natively, focus ring, aria
<Button variant="ghost" size="icon" onClick={handleClick}>
  <Check className="h-4 w-4" />
</Button>
```

### Don't: Use `ExternalLink` Icon for In-App Help Links

```tsx
// ❌ WRONG: ExternalLink implies opening a new tab/leaving the app
<HelpCircleIcon /> Open docs <ExternalLink className="h-3 w-3" />

// ✅ CORRECT: HelpCircle or BookOpen signals in-app modal
import { HelpLink } from "@/components/help/help-link";

// HelpLink already uses the correct icon internally — use it directly
<HelpLink articleId="my-article" />
```

See [UI Patterns: Avatar & Initials](../patterns/ui-patterns.md#avatar--initials), [UI Patterns: Loading States](../patterns/ui-patterns.md#loading-states).
