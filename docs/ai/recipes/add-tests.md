...
docker exec danavision-dev bash -c "cd /var/www/html/backend && php artisan test --filter=ExampleTest"

# Run with coverage
docker exec danavision-dev bash -c "cd /var/www/html/backend && php artisan test --coverage"
```
